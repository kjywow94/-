...
public class EthereumService implements IEthereumService {
		...
		List<Block> blocks = new ArrayList<Block>();	
	      ...
	      try {
		  	...
		  	for(int i =0; i < 20; i++) {
		 		...
		 		latestBlockResponse
		   		= web3j.ethGetBlockByNumber(targetId, true).sendAsync().get();
	...
	public List<EthereumTransaction> 최근트랜잭션조회()
	{
		...
		try {
			...
			transactionList = EthereumTransaction.getEthereumTransactionList(transactions, latestBlockResponse.getBlock().getTimestamp(), true);
	...
	public Block 블록검색(String 블록No)
	{
		...
		try {
			...
			latestBlockResponse
					= web3j.ethGetBlockByNumber(targetNum, true).sendAsync().get();
	...
	public EthereumTransaction 트랜잭션검색(String 트랜잭션Hash)
	{
		...
		try {
			...
			EthBlock.TransactionObject hash = new EthBlock.TransactionObject(data.getResult().getHash(),data.getResult().getNonceRaw(),data.getResult().getBlockHash(),
					data.getResult().getBlockNumberRaw(),data.getResult().getTransactionIndexRaw(),data.getResult().getFrom(),data.getResult().getTo(),data.getResult().getValueRaw(),
					data.getResult().getGasPriceRaw(),data.getResult().getGasRaw(),data.getResult().getInput(),data.getResult().getCreates(),data.getResult().getPublicKey(),data.getResult().getRaw(),data.getResult().getR(),data.getResult().getS(),(int)data.getResult().getV());
