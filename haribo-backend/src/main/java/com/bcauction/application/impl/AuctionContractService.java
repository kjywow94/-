...
public class AuctionContractService implements IAuctionContractService {
	...
	public AuctionInfo 경매정보조회(final String 컨트랙트주소)
	{
		...
		try {
				...
				LocalDateTime 경매시작시간 = LocalDateTime.ofInstant(Instant.ofEpochMilli(response경매시작시간), TimeZone.getDefault().toZoneId());
				LocalDateTime 경매종료시간 = LocalDateTime.ofInstant(Instant.ofEpochMilli(response경매종료시간), TimeZone.getDefault().toZoneId());
